package com.android.back4appchat.ui.screen.home
        ...
        query.findInBackground { chatRooms, error ->
            if (error == null) {
                if (chatRooms != null && chatRooms.isNotEmpty()) {
                    ...
                    _rooms.value = RequestState.Success(data = parsedChatRooms)
                } else {
                    _rooms.value = RequestState.Error(message = "Empty ChatRooms")
                }
            } else {
